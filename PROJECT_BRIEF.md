...
Resume Profile + Job Profile
→ `/api/match`
→ LLM semantic requirement comparison
→ deterministic TypeScript scoring
→ deterministic evidence-grounded resume suggestions
→ final Match Result
→ frontend display
...
The LLM does not calculate the final percentage and does not generate resume improvement wording in the current MVP.

Numerical scoring and resume improvement guidance are generated by deterministic application logic.
...
- English and German PDF support
...
- verification handling for uncertain requirements
- German and English suggestion output
- duplicate paid-request protection
- frontend final Match Result display
- scoring tests
- suggestion tests
- real Resume / JD testing
